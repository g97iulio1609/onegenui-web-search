...
var extractedImageSchema = z.object({
  src: z.string(),
...
var extractedVideoSchema = z.object({
  src: z.string(),
...
var extractedAudioSchema = z.object({
  src: z.string(),
  title: z.string().optional(),
  description: z.string().optional(),
  duration: z.number().optional()
...
var browserActionSchema = z.object({
  id: z.string(),
  action: z.enum([
    "navigating",
    "searching",
...
var scrapeResultSchema = z.object({
  url: z.string(),
  title: z.string(),
  content: z.string(),
...
var log = createLogger({ prefix: "web-search" });
...
var WebSearchUseCase = class {
   ...
   */
  async executeWithRetry(fn, adapterName) {
...
var OneCrawlScraperAdapter = class {
  scrapeUseCase = createScrapeUseCase();
  available = null;
  async scrape(url, options) {
    const startTime = Date.now();
...
import { createSearchUseCase } from "onecrawl";
var OneCrawlSearchAdapter = class {
  searchUseCase = createSearchUseCase();
  available = null;
  async search(query, options) {
    const startTime = Date.now();
    const oneCrawlOptions = {
...
var webSearchTool = defineMcpTool({
  name: "web-search",
  ...
  parameters: z2.object({
    ...
    maxResults: z2.number().min(1).max(20).optional().describe("Maximum number of results to return (default: 10)"),
    engine: z2.enum(["google", "duckduckgo", "bing"]).optional().describe("Search engine to use (default: duckduckgo)"),
    type: z2.enum(["web", "image", "video", "news"]).optional().describe("Type of search to perform (default: web)"),
    timeout: z2.number().min(5e3).max(3e5).optional().describe("Timeout in milliseconds (default: 60000)")
  }),
  domain: "web",
...
var webSnapshotTool = defineMcpTool({
  name: "web-snapshot",
  ...
  parameters: z2.object({
    url: z2.string().url().describe("The URL to snapshot")
  }),
  domain: "web",
...
var webHealthCheckTool = defineMcpTool({
